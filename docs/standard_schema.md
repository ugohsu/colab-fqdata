...
以下の 4 つのテーブルと 1 つのビューで構成されています。

## このデータベースについて（バージョン管理）

`standard.db` は年に 1 回程度、最新の FQ データから再構築されて配布される（本リポジトリには同梱されない）ファイルです。そのため、**「このDBがいつ・どの範囲のデータで作られたものか」は、このドキュメントには具体的な数値として書きません**（書くと再構築ごとに古くなり、実体とズレるため）。

配布された `standard.db` そのものの取得時点・収録期間・件数を確認したい場合は、必ず以下のいずれかを見てください:

* DB内の `Meta` テーブル（後述）を直接クエリする
* `FqLoader` で接続した際に自動表示される一行サマリ、または `loader.info()` の出力

論文等に「データの取得時点・観測数」を記載する必要がある場合も、この `Meta` テーブル（または `info()`）の値を引用してください。

過去の再構築で何が変わったか（スキーマ変更・不具合対応など）は [`docs/db_release_notes.md`](db_release_notes.md) を参照してください。
...
> **💡 取得対象企業（ユニバース）について**
> 本テーブルのソースデータは、**一般事業会社に加えて全ての上場企業（銀行・証券・保険を含む）**の企業プールをマージして重複排除（sort | uniq）したものです。後述の財務データ群よりも対象企業カバレッジが広くなっています。
...
> **⚠️ 取得対象企業について**
> 本テーブル（および後述の `Ratios` テーブル）のソースデータは、**一般事業会社（銀行・証券・保険を除いた有価証券報告書提出会社）のみ**が取得対象です。そのため、金融機関などの場合、`Companies` にマスタ情報が存在しても、こちらのテーブルにはデータが存在しない点にご注意ください。
...
| 決算期 | INTEGER | 決算年月 (YYYYMM の 6 ケタ整数) |
| 年度 | INTEGER | 会計年度 (Fiscal Year) ※5月決算以前は前年扱い |
| 証券コード | TEXT | 証券コード |
| 決算月数 | INTEGER | 決算対象期間の月数 (通常は12) |
| (BS/PL/CF/その他項目) | INTEGER | 財務諸表の各項目。**このテーブル定義は要約であり、全カラムの一覧は後述の「[勘定科目一覧](#勘定科目一覧-standard-テーブル詳細)」を正典とします**（本節には重複記載しません）。 |
...
3つのテーブルを結合し、決算月数が変則的なデータ（12ヶ月未満など）を除外した、分析用の主要ビューです。
基本的な財務分析は、まずこのビューを参照することを推奨します。
...
| --- | --- | --- |
| **NCODE** | Standard | 企業ID |
| **年度** | Standard | 会計年度 (Fiscal Year) |
| **企業名** | Companies | 企業名 |
| **証券コード** | Standard | 証券コード |
| **日経業種中分類名** | Companies | 日経業種名（中分類） |
| **日経業種小分類名** | Companies | 日経業種名（小分類） |
| **総資産** | Standard | 総資産（BS） |
| **売上高** | Standard | 売上高（PL） |
| **営業利益** | Standard | 営業利益（PL） |
| **当期純利益** | Standard | 当期純利益（PL） |
| **ROA** | Ratios | 総資産利益率 |
| **ROE** | Ratios | 自己資本利益率 |
| **ATO** | Ratios | 総資産回転率 |
| **PM** | Ratios | 売上高営業利益率 |
| **流動比率** | Ratios | 流動資産 / 流動負債 |
| **当座比率** | Ratios | 当座資産 / 流動負債 |
| **自己資本比率** | Ratios | 自己資本 / 総資産 |
| **有利子負債依存率** | Ratios | 有利子負債 / 総資産 |
| **総資本留保利益率** | Ratios | 利益剰余金 / 総資産 |
| **ICR** | Ratios | インタレスト・カバレッジ・レシオ |
| **経常収支比率** | Ratios | 経常的収入 / (経常的収入 - 営業CF) |
| **CICR** | Ratios | キャッシュフロー・インタレスト・カバレッジ・レシオ |
| **有利子負債返済年数** | Ratios | 有利子負債 / 営業CF |
| **std_id** | Standard | ユニークID（Standardテーブル結合用） |
| **決算期** | Standard | 決算年月 (YYYYMM) |

> **⚠️ Note (注意点):**
> * このビューには、上記の「収録カラム一覧」にある項目**のみ**が含まれています。
> * ビューに含まれていない詳細項目が必要な場合は、`std_id` をキーにして `Standard` テーブルを、`NCODE` をキーにして `Companies` テーブルを結合してください。

### 5. Meta（ビルド情報）

`standard.db` がいつ・どの範囲のデータから作られたかを記録した、1行だけのテーブルです。`standard.R` の実行時に自動生成されます（このテーブルが無い場合は、Meta 導入前の古いバージョンのDBです）。

`FqLoader` で接続すると、この内容が自動で1行サマリ表示されます。全項目を見たい場合は `loader.info()` を呼んでください。

| カラム名 | 型 | 説明 |
| --- | --- | --- |
| build_datetime | TEXT | `standard.db` を生成した日時 |
| script_version | TEXT | 生成に使った `standard.R` のバージョン（日付文字列。ロジック変更時に更新） |
| source_db | TEXT | 元データとして接続した SQLite ファイル名（例: `fq_all_data.db`） |
| fq_period_min | INTEGER | `Standard.決算期` の最小値（収録データの開始期、YYYYMM） |
| fq_period_max | INTEGER | `Standard.決算期` の最大値（収録データの終了期、YYYYMM） |
| n_companies_master | INTEGER | `Companies` テーブルの企業数（上場全社を含む広いユニバース） |
| n_companies_standard | INTEGER | `Standard` テーブルに財務データが存在する企業数 |
| n_observations | INTEGER | `Standard` テーブルの行数（観測数） |
| source_note | TEXT | データ源・利用条件に関する注記 |

> 📖 過去のバージョンで何が変わったか（スキーマ変更・不具合対応の履歴）は [`docs/db_release_notes.md`](db_release_notes.md) を参照してください。
...
| 前期末総資産 | ─ | ─ | **同一企業の前期（1期前）の総資産**。単独のFQ項目コードはなく、`shift_term()` により当該企業・当該決算期の1期前の `B01110`(総資産) をずらして取得した派生列。`ROA`・`自己資本比率`など多くの`Ratios`項目の計算に使われる。 |
| 総負債 | C01082 | 負債合計 | 負債合計。C01021(流動負債), C01057(固定負債), C01078(特別法上の準備金・引当金) の合計金額。 |
| 前期末総負債 | ─ | ─ | **同一企業の前期（1期前）の総負債**。単独のFQ項目コードはなく、`shift_term()` により当該企業・当該決算期の1期前の `C01082`(総負債) をずらして取得した派生列。`ROE`・`自己資本比率`など多くの`Ratios`項目の計算に使われる。 |
